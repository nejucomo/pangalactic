use crate::{
    store::{HostLink, HostReader},
    ByteReader, DirectoryReader, State,
};
    ...
    S: Store,
{
    ...
    let fr: HostReader<S> = caller.data_mut().store_mut().load(&link).await?;
...
{
    let link = caller.data().links().lookup(h_link)?.clone();
    let dr: DirectoryReader<S> = caller.data_mut().store_mut().load(&link).await?;
